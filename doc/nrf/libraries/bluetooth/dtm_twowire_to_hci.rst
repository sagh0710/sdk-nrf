...
`Bluetooth Core Specification`_, Vol 6, Part F, 2.1 Test sequences declares both an HCI and a 2-wire UART interface for Direct Test Mode (DTM), as shown in the table below.
...
Command and event translation follows the mappings in the table above.
When an error is detected in either a 2-wire UART command or an HCI event, the library instead produces an ``LE_Test_Status`` event with an error status.
...
* Not thread-safe.
  The caller must assure that calls to the library are serialized, for example by using a mutex or by calling from a single thread.
